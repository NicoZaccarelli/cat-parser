...
async function main() {
  console.log("=== Snapshot final Barcelona (post B4) ===");
  ...
  console.log(`Provincia 'Barcelona' : ${barcelona.toLocaleString("es-ES")}`);

  // Top 10 municipios de Barcelona por buildings (paginación + agregación)
  console.log("\n=== Top 10 municipios de provincia Barcelona por nº de buildings ===");
  ...
  console.log(`Total municipios distintos en Barcelona: ${top.length}`);
  console.log("Top 10:");
  top.slice(0, 10).forEach(([m, c], i) => {
    console.log(`  ${(i + 1).toString().padStart(2)}. ${m.padEnd(35)} : ${c.toLocaleString("es-ES")}`);
  });

  // Sample 3 buildings de un municipio top no-BARCELONA (capital)
  if (top.length >= 2) {
  ...
  console.log(`Barcelona (cap+resto) : ${barcelona}`);
}
