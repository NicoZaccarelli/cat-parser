# load-madrid.ps1 (v2 — endurecido tras cascada del v1)
# Cambios respecto a v1:
#   1) Sleep 3s entre iteraciones (Start-Sleep) para que el SO libere handles/memoria.
#   2) NO aborta al primer FAILED. Loguea, suma a $failedCodes y continúa.
#   3) Cola de retry al final del bucle: una vuelta extra sobre los FAILED con
#      sleep 30s antes de empezar y 5s entre cada retry.
#   4) Backoff extra de 5s ANTES de iniciar la siguiente iteración cuando la
#      anterior fue FAILED (mitiga la cascada ambiental observada en v1).

$ErrorActionPreference = "Continue"

$gzDir     = "E:\canScan\cat\madrid\28_U_23012026_CAT"
$unzipDir  = "$gzDir\unzipped"
$logsDir   = "E:\canScan\cat\madrid\logs"
$csvPath   = "$logsDir\madrid-summary.csv"
$parserDir = "D:\canScan\cat-parser"
$skip      = "28900U_23012026.CAT.gz"  # Madrid capital (ya cargado parcialmente)
...
Write-Host "Total municipios: $($files.Count) (excluye 28900U Madrid capital)"
...
Write-Host ""
Write-Host "================================================================="
Write-Host "=== RESUMEN MADRID v2 (excl. 28900U) ==="
